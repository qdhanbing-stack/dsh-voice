...
var TtsQueue = class {
  ...
  constructor(options = {}) {
    ...
    this.mutedSessions = /* @__PURE__ */ new Set();
  }
  cancelAll() {
    for (const [id, q] of this.queues) {
      q.epoch++;
      q.pending.length = 0;
      this.mutedSessions.add(id);
    }
  }
  ...
  enqueue(sessionId, text) {
    if (this.mutedSessions.has(sessionId)) return;
...
const SUMMARY_KEYWORDS = [
  "已完成", "成功", "搞定", "完成了", "执行完毕", "失败", "错误", "异常",
  "下一步", "接下来", "然后", "需要你", "请你", "请确认", "请看",
  "注意", "重要", "关键", "总结", "结论", "结果", "所以", "因此"
];
function extractSummarySentences(allSentences) {
  if (allSentences.length <= 2) return allSentences;
  const result = [];
  const added = new Set();
  const add = (s) => {
    const key = s.slice(0, 20);
    if (!added.has(key)) { added.add(key); result.push(s); }
  };
  if (allSentences[0]) add(allSentences[0]);
  for (const s of allSentences) {
    for (const kw of SUMMARY_KEYWORDS) {
      if (s.includes(kw)) { add(s); break; }
    }
  }
  ...
  for (const s of result) {
    if (totalChars + s.length > 250 && limited.length >= 2) break;
    limited.push(s);
    totalChars += s.length;
  }
  return limited;
...
async function* tapStream(sessionId, inner, queue, sessionSegmenters) {
  queue.mutedSessions.delete(sessionId);
  ...
  try {
    for await (const chunk of inner) {
      if (chunk.type === "text-delta" && chunk.text) {
        const newSents = segmenter.feed(chunk.text);
        for (const s of newSents) allSentences.push(s);
      }
    ...
    if (!aborted) {
      for (const s of segmenter.flush()) allSentences.push(s);
      const summary = extractSummarySentences(allSentences);
      for (const s of summary) queue.enqueue(sessionId, s);
    }
