...
function createMicBridge() {
  const listeners = /* @__PURE__ */ new Set();
  let state = {
    asrState: "idle", error: null, holding: false, levels: [],
    partial: "", pending: false, hotkey: "", armedCancel: false,
    indicator: null, label: "mic"
  };
  return {
    actionsRef: { current: { toggle: () => {}, holdProps: {} } },
    subscribe(fn) { listeners.add(fn); fn(state); return () => listeners.delete(fn); },
    setState(patch) {
      const next = { ...state, ...patch };
      let changed = false;
      for (const k in patch) { if (next[k] !== state[k]) { changed = true; break; } }
      if (!changed) return;
      state = next;
...
function apply(ctx) {
  ...
  const micBridge = createMicBridge();
  ctx.slots.inject(
    "shell.overlay",
    () => ctx.slots.register(
      {
        ...
        id: "voice",
        order: 100,
        inject: () => ({ ...engine, setEnabled: engine.setEnabled, setTtsEnabled: engine.setTtsEnabled, micBridge })
      },
      VoicePanel
    )
  );
...
function createAudioEngine() {
  const listeners = /* @__PURE__ */ new Set();
  let state = { connected: false, playing: false, caption: null, enabled: true };
  const queue = [];
  ...
  const loadEnabled = () => {
    try {
      const v = localStorage.getItem("dsh-voice-enabled");
      return v === null ? true : v === "1";
    } catch { return true; }
  };
  const saveEnabled = (v) => {
    try { localStorage.setItem("dsh-voice-enabled", v ? "1" : "0"); } catch {}
  };
  const loadTtsEnabled = () => {
    try { return localStorage.getItem("dsh-voice-tts") !== "0"; } catch { return true; }
  };
  const saveTtsEnabled = (v) => {
    try { localStorage.setItem("dsh-voice-tts", v ? "1" : "0"); } catch {}
  };
  state.enabled = loadEnabled();
  state.ttsEnabled = loadTtsEnabled();
  let suppressUntil = 0;
  let voiceCmd = null;
  const stopVoiceCmd = () => {
    if (voiceCmd) { try { voiceCmd.stop(); } catch {} voiceCmd = null; }
  };
  const startVoiceCmd = () => {
    ...
    try {
      voiceCmd = new SR();
      voiceCmd.lang = "zh-CN";
      voiceCmd.continuous = true;
      voiceCmd.interimResults = true;
      voiceCmd.onresult = (ev) => {
        let t = "";
        for (let i = ev.resultIndex; i < ev.results.length; i++) t += ev.results[i][0].transcript;
        if (/停|stop|闭嘴|安静/i.test(t)) stopAll();
      };
      voiceCmd.onerror = () => {};
      voiceCmd.onend = () => { if (state.playing) try { voiceCmd.start(); } catch {} };
      voiceCmd.start();
    } catch {}
  };
  const stopAll = () => {
    ...
    stopVoiceCmd();
    void fetch("/dsh-voice-api/cancel", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" }).catch(() => {});
  };
  const playNext = () => {
    if (!state.ttsEnabled) { queue.length = 0; setState({ playing: false, caption: null }); return; }
    ...
    if (!voiceCmd) startVoiceCmd();
    void audio.play().catch(() => { setState({ playing: false }); });
  };
  const disconnect = () => {
  ...
  const connect = () => {
    if (!state.enabled) { disconnect(); return; }
    if (source) return;
    ...
    source.addEventListener("audio", (e) => {
      if (!state.enabled || !state.ttsEnabled || Date.now() < suppressUntil) return;
  ...
  if (typeof window !== "undefined") {
    window.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && state.playing) { e.preventDefault(); stopAll(); }
    });
  }
  const setEnabled = (v) => {
    const enabled = !!v;
    saveEnabled(enabled);
    try { window.dispatchEvent(new CustomEvent("dsh-voice-enabled-change", { detail: { enabled } })); } catch {}
    if (enabled) {
      ...
      stopVoiceCmd();
      disconnect();
      setState({ enabled: false, playing: false, caption: null });
      void fetch("/dsh-voice-api/cancel", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" }).catch(() => {});
    }
  };
  const setTtsEnabled = (v) => {
    const ttsEnabled = !!v;
    ...
    if (!ttsEnabled) {
      ...
      audio.onerror = null;
      stopVoiceCmd();
      setState({ ttsEnabled: false, playing: false, caption: null });
      void fetch("/dsh-voice-api/cancel", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" }).catch(() => {});
    } else {
      setState({ ttsEnabled: true });
    }
  };
  return { connect, skip, subscribe, stopAll, setEnabled, setTtsEnabled };
}
function VoicePanel(props) {
  const { connect, skip, subscribe, setEnabled, setTtsEnabled, micBridge } = props;
  ...
  const dragRef = (0, import_react.useRef)({ dragging: false, startX: 0, startY: 0, origLeft: 0, origTop: 0 });
  const panelRef = (0, import_react.useRef)(null);
  const loadPos = () => {
    try {
      const v = localStorage.getItem("dsh-voice-pos");
      if (!v) return null;
  ...
  const savePos = (left, top) => {
    try { localStorage.setItem("dsh-voice-pos", JSON.stringify({ left, top })); } catch {}
  };
  ...
  (0, import_react.useEffect)(() => {
    const onMove = (e) => {
      ...
      let nl = dragRef.current.origLeft + dx;
      let nt = dragRef.current.origTop + dy;
    ...
    const onUp = () => {
      ...
      if (pos) savePos(pos.left, pos.top);
    };
  ...
  const [micState, setMicState] = (0, import_react.useState)(null);
  (0, import_react.useEffect)(() => {
    if (!micBridge) return;
    return micBridge.subscribe(setMicState);
  }, [micBridge]);
  useStyle(UI_CSS);
  const [discussMode, setDiscussMode] = (0, import_react.useState)(() => {
    try { return localStorage.getItem("dsh-voice-discuss") === "1"; } catch { return false; }
  });
  const toggleDiscuss = () => {
    const next = !discussMode;
    setDiscussMode(next);
    try { localStorage.setItem("dsh-voice-discuss", next ? "1" : "0"); } catch {}
    if (micBridge) micBridge.setState({ ...micBridge.micStateRef, discussMode: next });
  };
  const toggleTts = () => { if (setTtsEnabled) setTtsEnabled(!state.ttsEnabled); };
  const toggleRec = () => { micBridge?.actionsRef.current?.toggle?.(); };
  const isRecording = micState && (micState.asrState === "recording" || micState.holding);
  const playing = state.connected && state.playing && state.enabled && state.ttsEnabled;
  const dot = !state.enabled ? "#6b7280" : playing ? "#2ea043" : state.connected ? "#8b949e" : "#f85149";
  const posStyle = pos ? { left: pos.left, top: pos.top, right: "auto", bottom: "auto" } : { right: 16, bottom: 16 };
      ...
      children: [
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          "span",
          {
            ...
            children: !state.enabled ? "语音已关闭" : discussMode ? "\u8BA8\u8BBA\u6A21\u5F0F - \u8BF4\u201C\u5F00\u59CB\u6267\u884C\u201D\u5F00\u59CB" : !state.ttsEnabled ? "播报已关闭" : state.caption ?? (state.connected ? "voice ready" : "voice offline")
          },
          state.caption ?? "idle"
        ),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          "button",
          {
            className: "dshv-icon-btn dshv-rec-btn",
            onClick: (e) => { e.stopPropagation(); toggleRec(); },
            ...micBridge?.actionsRef.current?.holdProps || {},
            title: isRecording ? "\u505C\u6B62\u5F55\u97F3" : "\u5F00\u59CB\u5F55\u97F3",
            style: {
              border: "none",
              background: isRecording ? "rgba(248, 81, 73, 0.3)" : "rgba(107, 114, 128, 0.15)",
              color: isRecording ? "#f85149" : "#e6e8eb",
              cursor: "pointer",
              flexShrink: 0,
              transition: "background 0.2s ease, color 0.2s ease"
            },
            dangerouslySetInnerHTML: { __html: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="3" width="6" height="12" rx="3" fill="${isRecording ? "currentColor" : "none"}"/><path d="M5 11a7 7 0 0 0 14 0"/><line x1="12" y1="18" x2="12" y2="22"/></svg>` }
          }
        ),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          "button",
          {
            className: "dshv-icon-btn dshv-tts-btn",
            onClick: (e) => { e.stopPropagation(); toggleTts(); },
            title: state.ttsEnabled ? "\u5173\u95ED\u64AD\u62A5" : "\u5F00\u542F\u64AD\u62A5",
            style: {
              border: "none",
              background: state.ttsEnabled ? "rgba(46, 160, 67, 0.25)" : "rgba(107, 114, 128, 0.15)",
              color: state.ttsEnabled ? "#2ea043" : "#8b949e",
              cursor: "pointer",
              flexShrink: 0,
              transition: "background 0.2s ease, color 0.2s ease"
            },
            dangerouslySetInnerHTML: { __html: state.ttsEnabled
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          "button",
          {
            className: `dshv-discuss-btn${discussMode ? "" : " off"}`,
            onClick: (e) => { e.stopPropagation(); toggleDiscuss(); },
            title: discussMode ? "\u9000\u51FA\u8BA8\u8BBA\u6A21\u5F0F" : "\u5F00\u542F\u8BA8\u8BBA\u6A21\u5F0F",
            style: {
              border: "none",
              borderRadius: 999,
              fontSize: 12,
...
.dshv-skip:hover { background: rgba(248, 81, 73, 1) !important }
.dshv-mic:hover { background: rgba(139, 148, 158, 0.14) !important }
.dshv-mic { touch-action: none }
@keyframes dshv-discuss-glow {
  0% { background-position: 0% 50%; box-shadow: 0 0 8px rgba(139, 92, 246, 0.4), 0 0 16px rgba(59, 130, 246, 0.2) }
  25% { box-shadow: 0 0 12px rgba(6, 182, 212, 0.5), 0 0 24px rgba(139, 92, 246, 0.3) }
  50% { background-position: 100% 50%; box-shadow: 0 0 8px rgba(34, 197, 94, 0.4), 0 0 16px rgba(6, 182, 212, 0.2) }
  75% { box-shadow: 0 0 12px rgba(249, 115, 22, 0.5), 0 0 24px rgba(34, 197, 94, 0.3) }
  100% { background-position: 0% 50%; box-shadow: 0 0 8px rgba(139, 92, 246, 0.4), 0 0 16px rgba(59, 130, 246, 0.2) }
}
.dshv-discuss-btn {
  background: linear-gradient(90deg, #8b5cf6, #06b6d4, #22c55e, #f97316, #8b5cf6);
  background-size: 300% 300%;
  animation: dshv-discuss-glow 3s ease infinite;
  color: #fff !important;
...
.dshv-discuss-btn > * { position: relative; z-index: 1; }
.dshv-discuss-btn.off {
  animation: none;
  background: rgba(107, 114, 128, 0.2) !important;
  color: #6b7280 !important;
  box-shadow: none !important;
}
.dshv-discuss-btn.off::before { background: transparent; }
.dshv-icon-btn {
...
.dshv-icon-btn svg { width: 18px; height: 18px; display: block; }
...
function MicButton({ useSession, useInput, inputActions, skipPlayback, cancelTurn, micBridge }) {
  const [voiceEnabled, setVoiceEnabled] = (0, import_react.useState)(() => {
    try { return localStorage.getItem("dsh-voice-enabled") !== "0"; } catch { return true; }
  });
  (0, import_react.useEffect)(() => {
    const handler = (e) => { setVoiceEnabled(e.detail.enabled); };
    window.addEventListener("dsh-voice-enabled-change", handler);
    return () => window.removeEventListener("dsh-voice-enabled-change", handler);
  }, []);
  ...
  const discussModeRef = (0, import_react.useRef)(false);
  (0, import_react.useEffect)(() => {
    if (!micBridge) return;
    return micBridge.subscribe((s) => { discussModeRef.current = !!s.discussMode; });
  }, [micBridge]);
  ...
  (0, import_react.useEffect)(() => {
    ...
    fetch("/dsh-voice-api/config").then((r) => r.json()).then((c) => {
      ...
      engine.onSegment((text) => {
        ...
        let next = current.trim() === "" ? trimmed : current.replace(/\s+$/, "") + " " + trimmed;
        let submitNow = submitOnTranscriptRef.current || c.asr.autoSend;
        submitOnTranscriptRef.current = false;
        if (discussModeRef.current) {
          submitNow = false;
          const cmdPattern = /(开始执行|可以了|开始做|开始吧|就这么办|执行吧|好的开始)/;
          const match = next.match(cmdPattern);
          if (match) {
            next = next.replace(match[0], "").replace(/\s+/g, " ").trim();
            if (!next && current.trim()) next = current.trim();
            submitNow = true;
            discussModeRef.current = false;
            if (micBridge) micBridge.setState({ discussMode: false });
            try { localStorage.setItem("dsh-voice-discuss", "0"); } catch {}
          }
        }
  ...
  if (micBridge) {
    micBridge.actionsRef.current = { toggle, holdProps };
  }
  (0, import_react.useEffect)(() => {
    if (!micBridge) return;
    micBridge.setState({ asrState, error, holding, levels, partial, pending, hotkey, armedCancel, indicator, label });
  });
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { ref: anchorRef, style: { display: "none" } })
  ] });
}
